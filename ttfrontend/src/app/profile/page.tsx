    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center p-4 bg-[#1b1b1b]">
        ...
        <div className="w-full max-w-lg bg-[#2b0f1a]/50 p-8 rounded-xl border border-[#7a2f49] shadow-lg text-center z-10">
          ...
          <h2 className="font-vt323 text-3xl text-center text-[#ffdcdc] mb-2">// VERIFICATION REQUIRED</h2>
          <p className="font-mono text-center text-gray-300 mb-6">A verification link has been sent to <strong className="text-yellow-400">{user.email}</strong>. Please check your inbox (and spam folder) to continue.</p>
  ...
  return (
    <div className="min-h-screen bg-[#1b1b1b] p-4 sm:p-8 lg:p-12">
      ...
      <header className="flex justify-between items-center pb-8 border-b border-[#ef3b57]/20 mb-8">
        ...
        <button onClick={handleLogout} className="font-mono text-sm text-[#d9bfc6] flex items-center gap-2 px-3 py-1 border border-transparent rounded-md hover:border-[#ef3b57]/50 hover:bg-[#2f1f2b] transition-all">
            <LogOut size={16} /> Sign Out
        </button>
      </header>
      <main className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
        <div className="lg:col-span-5"> <UserProfileCard user={displayUser} /> </div>
        <div className="lg:col-span-7">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            <div className="col-span-2 md:col-span-3"> <StatItem label="Total Points" value={displayStats.totalPoints.toLocaleString()} icon={<Gem size={32} />} isPrimary={true} /> </div>
